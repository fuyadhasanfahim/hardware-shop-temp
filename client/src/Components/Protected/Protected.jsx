...
const Protected = ({ children }) => {
  const { user, loading, tokenReady } = useContext(ContextData);
  const location = useLocation();

  if (loading || (user && !tokenReady)) {
    // Show spinner while loading or waiting for token
    return (
  ...
  if (user && tokenReady) {
    // If user is logged in and token is ready, render children components
    return children;
  }
